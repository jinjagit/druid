...
pub fn main() {
    let window = WindowDesc::new(make_ui).title(
        LocalizedString::new("identity-demo-window-title").with_placeholder("My External Event Demo"),
    );
...
fn make_ui() -> impl Widget<MyColor> {
    let colorthing = ColorWell::new()
        .fix_width(300.0)
        .fix_height(300.0)
        .padding(10.0)
        .center();

    let label = Label::new(|data: &MyColor, _env: &Env| format!("color: {:?}!", data.0));

    // arrange the two widgets vertically, with some padding
    let layout = Flex::column()
        .with_child(colorthing)
